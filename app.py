...
# Function to fetch a random passage from the Forismatic API
def get_random_passage():
    try:
        response = requests.get("http://api.forismatic.com/api/1.0/?method=getQuote&format=json&lang=en", timeout=5)
        response.raise_for_status()  # Raise an exception for HTTP errors
        data = response.json()
        return data.get('quoteText', "Unable to fetch quote.")  # Return only the quote text
    except requests.exceptions.RequestException as e:
        print(f"Error fetching random passage: {e}")
...
def home():
    ...
    random_passage = get_random_passage()  # Fetch a random passage
